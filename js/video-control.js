...
function pressKeyButton(key) {
  if (
    (window.pageYOffset >= videoSection.offsetTop - 100 &&
      window.pageYOffset <
        videoSection.offsetTop + videoSection.getBoundingClientRect().height) ||
...
document.addEventListener("keypress", (e) => {
  if (
    ...
    const keyName = e.key;
    if (
      (e.shiftKey && e.key.toLowerCase() === "<") ||
      (e.shiftKey && e.key.toLowerCase() === "б")
    ) {
      toggleSpeedUp();
    }
    if (
      (e.shiftKey && e.key.toLowerCase() === ">") ||
      (e.shiftKey && e.key.toLowerCase() === "ю")
    ) {
      toggleSpeedDown();
    }
